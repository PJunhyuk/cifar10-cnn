if __name__ == "__main__":
    ...
    class Net2Wider2Net(nn.Module):
        def __init__(self):
            # 64 * 1.2 = 77
            # 128 * 1.2 = 154
            # 154 * 8 * 8 = 9856
            super(Net2Wider2Net, self).__init__()
            self.conv1 = nn.Conv2d(3, 77, 5, padding=2) # 32 -> 32
            self.conv2 = nn.Conv2d(77, 77, 5, padding=2) # 16 -> 16
            self.conv3 = nn.Conv2d(77, 154, 3, padding=1) # 8 -> 8
            self.conv4 = nn.Conv2d(154, 154, 3, padding=1) # 8 -> 8
            self.conv5 = nn.Conv2d(154, 154, 3, padding=1) # 8 -> 8
            self.pool = nn.MaxPool2d(2, 2)
            self.fc1 = nn.Linear(9856, 384)
            self.fc2 = nn.Linear(384, 10)
        ...
        def forward(self, x):
            ...
            x = F.relu(self.conv4(x)) ## 8x8x128 -> 8x8x128
            x = F.relu(self.conv5(x)) ## 8x8x128 -> 8x8x128
            x = x.view(-1, 8*8*154) ## 8x8x128 -> 8192
            x = F.relu(self.fc1(x)) ## 8192 -> 384
            x = F.dropout(x, training=self.training)
            x = self.fc2(x) ## 384 -> 10
    ...
    for i in range(0, 64):
        for j in range(0, 3):
            net._modules['conv1'].weight.data[i][j] = net_teacher._modules['conv1'].weight.data[i][j]

    for i in range(0, 64):
        for j in range(0, 64):
            net._modules['conv2'].weight.data[i][j] = net_teacher._modules['conv2'].weight.data[i][j]

    for i in range(0, 128):
        for j in range(0, 64):
            net._modules['conv3'].weight.data[i][j] = net_teacher._modules['conv3'].weight.data[i][j]

    for i in range(0, 128):
        for j in range(0, 128):
            net._modules['conv4'].weight.data[i][j] = net_teacher._modules['conv4'].weight.data[i][j]

    for i in range(0, 128):
        for j in range(0, 128):
            net._modules['conv5'].weight.data[i][j] = net_teacher._modules['conv5'].weight.data[i][j]
    ...
    for epoch in range(epoch_max):
        ...
        filename = './checkpoint/checkpoint_wider2_base_' + str(epoch) + '.pth'
